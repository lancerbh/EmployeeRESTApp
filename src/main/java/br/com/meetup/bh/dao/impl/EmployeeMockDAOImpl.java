...
package br.com.meetup.bh.dao.impl;
...
import br.com.meetup.bh.mock.MockEmployeeList;
...
public class EmployeeMockDAOImpl implements EmployeeDAO {
	...
	public boolean add(Employee employee) {
		...
		Employee nextEmployee = new Employee(next, employee.getFirstName(), employee.getLastName(), employee.getEmail(),
	...
	public boolean update(long id, Employee employee) {
		int matchIndex = -1;
		matchIndex = eList.stream().filter(e -> e.getId() == id).findFirst().map(e -> eList.indexOf(e))
				.orElse(matchIndex);
		if (matchIndex > -1) {
